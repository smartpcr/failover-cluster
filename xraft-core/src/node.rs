...
impl RaftNode {
    ...
    ///
    /// **Stale-term contract:** passing `term < self.current_term()` is a
    /// caller bug. Raft §5.1 requires a node to ignore any RPC carrying a
    /// term lower than its own and *never* acknowledge a leader from a
    /// stale term. We enforce that here defensively: the call is a no-op
    /// (no role change, no leader recorded, no timer reset, no actions
    /// emitted) so an upstream mistake cannot corrupt the local state or
    /// propagate a wrong leader hint to clients. A `debug_assert!` fires
    /// in test builds to catch the miswire, and a `tracing::warn!` is
    /// always emitted so the drop is observable in production.
    ///
    ...
    pub fn become_follower(&mut self, term: Term, leader_id: Option<NodeId>) -> Vec<Action> {
        // Stale-term guard (Raft §5.1): a node MUST NOT acknowledge or step
        // down for a term older than its own. Ignore the call entirely so
        // role/leader_id/vote tallies/timer all stay intact and we don't
        // hand clients a phantom leader hint from a bygone term.
        if term < self.hard_state.current_term {
            debug_assert!(
                false,
                "become_follower called with stale term {} < current_term {}",
                term.0, self.hard_state.current_term.0
            );
            tracing::warn!(
                node_id = %self.id,
                stale_term = %term,
                ...
                ?leader_id,
                "ignoring become_follower with stale term (Raft §5.1)"
            );
            return Vec::new();
        }
...
"#,
    ...
    #[test]
    fn become_follower_with_stale_term_is_rejected() {
        // Raft §5.1 invariant: a node must NEVER acknowledge a leader from a
        // term older than its own. `become_follower(stale_term, _)` is a
        // caller bug, so we no-op: role unchanged, leader_id unchanged,
        // vote tallies and election timer untouched, no actions emitted.
        // The reverse — silently recording NodeId(99) as leader of term 7
        // because some upstream code passed Term(3) — would propagate a
        // bogus leader hint to clients and violate the safety property.
        //
        // The `debug_assert!` in `become_follower` would fire in test
        // builds, so we route this through `std::panic::catch_unwind` to
        // observe the post-condition (state untouched, no actions) without
        // failing the test on the assert.
        use std::panic::AssertUnwindSafe;

        // Drive the node to term=5 as a Candidate, then have it acknowledge
        // a real leader at term=5 so leader_id and role are well-defined.
        let mut node = RaftNode::new_with_seed(three_voter_config(), 1).unwrap();
        for _ in 0..5 {
            let _ = node.become_candidate();
        }
        let _ = node.become_follower(Term(5), Some(NodeId(2)));
        assert_eq!(node.current_term(), Term(5));
        assert_eq!(node.leader_id, Some(NodeId(2)));
        assert_eq!(node.role, NodeRole::Follower);
        ...
        // Stale-term call: node is at term 5, caller passes term 3 with a
        // bogus leader hint. Wrap in catch_unwind so the in-test
        // `debug_assert!` does not fail the test — we want to inspect the
        // post-state (which the early-return guarantees stays intact even
        // when the assert is compiled out in release builds).
        let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
            node.become_follower(Term(3), Some(NodeId(99)))
        }));

        // Whether the debug-assert fired or not, the early-return must
        // have produced no actions and left every field untouched.
        if let Ok(actions) = result {
            assert!(
                actions.is_empty(),
                "stale-term become_follower must emit no actions, got {actions:?}"
            );
        }
        assert_eq!(
            node.current_term(),
            term_before,
            "stale-term call must not mutate current_term"
        );
        assert_eq!(
            node.hard_state.voted_for, voted_for_before,
            "stale-term call must not clear vote"
        );
        assert_eq!(
            node.leader_id, leader_before,
            "stale-term call must NOT record NodeId(99) as the leader \
             — that would propagate a bogus leader hint to clients"
        );
        assert_ne!(
            node.leader_id,
            Some(NodeId(99)),
            "explicit guard against the exact bug from the review comment"
        );
        assert_eq!(
            node.role, role_before,
            "stale-term call must not change role"
        );
        assert_eq!(
            node.election_timer.remaining(),
            timer_remaining_before,
            "stale-term call must not reset the election timer"
        );
    }
