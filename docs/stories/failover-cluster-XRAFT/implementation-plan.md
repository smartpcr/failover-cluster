...
- [ ] Define `ClusterConfig` struct in `xraft-core/src/config.rs` with all fields that `architecture.md` §8 designates as `ClusterConfig` fields: `node_id`, `cluster_id`, `data_dir`, `listen_addr`, `peers` (Vec of peer addresses), `bootstrap_voters` (Vec of `{node_id, host, port}` — initial voter set parsed into `VoterSet` at startup), `bootstrap_observers` (Vec of `{node_id, host, port}` — initial observer set), `election_timeout_min_ms`, `election_timeout_max_ms`, `tick_interval_ms`, `fetch_interval_ms`, `snapshot_interval`, `max_log_entries_before_compaction`, `enable_check_quorum` (default true), `enable_leader_lease` (default false), `segment_max_bytes` (default 64 MiB — max WAL segment file size per Stage 2.1), `snapshot_max_chunk_bytes` (default 1 MiB — max chunk size for streamed FetchSnapshot transfers), `tls_enabled` (default false), `tls_cert_path`, `tls_key_path` (per `tech-spec.md` §2.7 / `architecture.md` §2.3 — not mandatory for v1 functional correctness but the configuration surface must exist)
...
- [ ] Define `EntryPayload` enum in `xraft-core/src/types.rs` with variants: `Command(Bytes)`, `NoOp`, `Snapshot(SnapshotMeta)`, `ConfigChange(VoterSet)` — these are the canonical Rust-side entry payload variants (per `architecture.md` §2.1); `Snapshot` carries snapshot metadata for compaction markers; `ConfigChange` is reserved for forward-compatible log format but `RaftNode` rejects any `ConfigChange` proposal with `UNSUPPORTED` in v1 (dynamic membership is out of scope); define `SnapshotMeta` as `{ last_included_index: LogIndex, last_included_term: Term }`
...
- [ ] Scenario: entry-payload-variants — Given `Entry` values constructed with each `EntryPayload` variant (`Command`, `NoOp`, `Snapshot`, `ConfigChange`), When pattern-matched, Then each variant is correctly identified and inner data is accessible
...
### Implementation Steps
- [ ] Implement CLI argument parsing in `xraft-server/src/main.rs` using `clap` or manual `std::env::args`: accept `--config <path>` (required — path to TOML config file) and `--node-id <id>` (optional — override node ID from config); validate that the config path exists and print a usage message on invalid arguments
