...
- [ ] Implement leader lease optimization: if the leader has heard from a majority within the last election timeout period (via incoming Fetch RPCs), it may skip the extra commit-index confirmation round-trip when answering internal read queries (e.g. admin status queries or `StateMachine`-based lookups); this is a leader-internal optimization and does not expose an external client read API
...
- [ ] Implement `AddVoter`/`RemoveVoter` command rejection: any attempt to issue an `AddVoter` or `RemoveVoter` command must return an `UNSUPPORTED` error with a message indicating dynamic membership is **out of scope for v1** and deferred to a future story entirely — all sibling documents now agree on this strict scoping: `tech-spec.md` §2.7 says "out of scope for v1 and deferred to a future story entirely — it is not a stretch goal within XRAFT", `architecture.md` §2.1/§10 says "out of scope for v1 and deferred to a future story entirely" (updated from earlier "stretch goal" language), and `e2e-scenarios.md` Feature 12 adopts the same wording; the voter set remains static for v1
