...
- [ ] Add shared workspace dependencies in root `Cargo.toml`: `tokio`, `serde`, `serde_json`, `tracing`, `tracing-subscriber`, `thiserror`, `bytes`, `prost`, `tonic`, `uuid`, `toml`, `rand`, `crc32fast`, `axum`, `prometheus-client`, `clap` (with `derive` feature — used by `xraft-server` for CLI argument parsing in Stage 6.1); add `tonic-build` and `prost-build` as workspace build-dependencies (used by `xraft-core/build.rs` in Stage 1.3 for protobuf code generation)
- [ ] Add `rust-toolchain.toml` pinning stable channel (e.g. `channel = "stable"`); set `edition = "2024"` in each crate's `Cargo.toml` manifest
- [ ] Add `.gitignore` for Rust (`/target`); **commit** the root `Cargo.lock` (do not ignore it) because this workspace contains the deployable `xraft-server` binary, where reproducible dependency locking is expected — `Cargo.lock` is only typically ignored for pure library crates
...
- [ ] Define `ClusterConfig` struct in `xraft-core/src/config.rs` with all fields that `architecture.md` §8 designates as `ClusterConfig` fields: `node_id`, `cluster_id`, `data_dir`, `listen_addr`, `peers` (Vec of peer addresses), `bootstrap_voters` (Vec of `{node_id, host, port}` — initial voter set parsed into `VoterSet` at startup), `bootstrap_observers` (Vec of `{node_id, host, port}` — initial observer set), `election_timeout_min_ms`, `election_timeout_max_ms`, `tick_interval_ms`, `fetch_interval_ms`, `snapshot_interval`, `max_log_entries_before_compaction`, `enable_check_quorum` (default true), `enable_leader_lease` (default false), `segment_max_bytes` (default 64 MiB — max WAL segment file size per Stage 2.1), `snapshot_max_chunk_bytes` (default 1 MiB — max chunk size for streamed FetchSnapshot transfers), `snapshot_retention_count` (u32, default 3 — number of most-recent snapshots to retain after cleanup; used by `FileSnapshotStore` in Stage 2.3), `tls_enabled` (default false), `tls_cert_path`, `tls_key_path` (per `tech-spec.md` §2.7 / `architecture.md` §2.3 — not mandatory for v1 functional correctness but the configuration surface must exist)
...
- [ ] Define `Input` enum in `xraft-core/src/types.rs` with variants: `Tick`, `VoteRequest(VoteRequest)`, `VoteResponse { from: NodeId, resp: VoteResponse }`, `PreVoteRequest(PreVoteRequest)`, `PreVoteResponse { from: NodeId, resp: PreVoteResponse }`, `FetchRequest(FetchRequest)`, `FetchResponse(FetchResponse)`, `ClientPropose { data: Bytes }`, `SnapshotComplete { metadata: SnapshotMeta }`, `SnapshotInstalled { metadata: SnapshotMeta }` — all inputs to the pure `RaftNode::step()` function; the base set (`Tick`, `VoteRequest`, `VoteResponse`, `FetchRequest`, `FetchResponse`, `ClientPropose`) is listed in `architecture.md` §2.1; this plan extends it with `PreVoteRequest`, `PreVoteResponse`, `SnapshotComplete`, and `SnapshotInstalled` to fully support the Pre-Vote protocol (§3.2) and snapshot lifecycle (§5) — these extensions are implementation-level detail that the architecture table summarises under higher-level concepts
- [ ] Define `Action` enum in `xraft-core/src/types.rs` with variants: `PersistHardState(HardState)`, `AppendEntries(Vec<Entry>)`, `SendMessage { to: NodeId, message: OutboundMessage }`, `ApplyToStateMachine { index: LogIndex, data: Bytes }`, `TakeSnapshot { through_index: LogIndex }`, `InstallSnapshot { metadata: SnapshotMeta, data: Bytes }`, `BecomeLeader`, `StepDown { term: Term }`, `RedirectToSnapshot { follower_id: NodeId, snapshot_metadata: SnapshotMeta }`, `TruncateLog { before_index: LogIndex }` — all side-effects emitted by `RaftNode::step()`; the base set (`PersistHardState`, `AppendEntries`, `SendMessage`, `ApplyToStateMachine`, `TakeSnapshot`, `InstallSnapshot`, `BecomeLeader`, `StepDown`) is listed in `architecture.md` §2.1; this plan extends it with `RedirectToSnapshot` (leader tells driver to initiate snapshot transfer when a follower is too far behind) and `TruncateLog` (driver truncates WAL after snapshot) to fully support the snapshot lifecycle (§5) — these extensions are implementation-level detail that the architecture table covers narratively in §5.3; define `OutboundMessage` enum with variants `Vote`, `PreVote`, `Fetch`, `FetchSnapshot` wrapping the respective request/response types
...
- [ ] Add unit tests for config deserialization, default values, `VoterSet` quorum calculations, and `Entry`/`EntryPayload` construction

### Dependencies
- phase-project-scaffolding/stage-cargo-workspace-and-crate-layout

### Test Scenarios
- [ ] Scenario: node-config-from-toml — Given a valid TOML config string with all fields, When deserialized into `NodeConfig`, Then all fields match expected values including the embedded `ClusterConfig` fields (`bootstrap_voters`, `segment_max_bytes`, `enable_check_quorum`, `tls_enabled`, peer addresses) and `NodeConfig`-only fields (`connect_timeout_ms`, `request_timeout_ms`, `admin_listen_addr`)
- [ ] Scenario: config-defaults — Given a minimal TOML with only required fields, When deserialized into `NodeConfig`, Then optional fields use sensible defaults (election_timeout_min_ms=150, fetch_interval_ms=50, enable_check_quorum=true, segment_max_bytes=64 MiB, snapshot_retention_count=3, connect_timeout_ms=500)
- [ ] Scenario: node-config-to-cluster-config — Given a fully populated `NodeConfig`, When `into_cluster_config()` is called, Then the resulting `ClusterConfig` contains all consensus fields (`node_id`, `cluster_id`, `data_dir`, `listen_addr`, `peers`, timing, `bootstrap_voters`, `bootstrap_observers`, `enable_check_quorum`, `enable_leader_lease`, `segment_max_bytes`, `snapshot_max_chunk_bytes`, `snapshot_retention_count`, `tls_*`, snapshot/compaction) and the `NodeConfig`-only fields (`connect_timeout_ms`, `request_timeout_ms`, `admin_listen_addr`) are not present in `ClusterConfig`
...
### Implementation Steps
- [ ] Create `proto/raft.proto` defining protobuf messages for all wire RPCs: `VoteRequest`, `VoteResponse`, `PreVoteRequest`, `PreVoteResponse`, `FetchRequest`, `FetchResponse`, `FetchSnapshotRequest`, `FetchSnapshotChunk` — these are the complete set of wire RPC messages (election, log replication, and snapshot transfer); `Action::AppendEntries` is a pure Rust enum variant in `xraft-core` representing the internal side-effect of the leader appending to its own log and has no protobuf or gRPC representation (per `tech-spec.md` §2.2 and `architecture.md` §2.1)
...
- [ ] Implement snapshot cleanup: retain only the N most recent snapshots, where N is `ClusterConfig::snapshot_retention_count` (defined in Stage 1.2, default 3); `FileSnapshotStore` receives this value at construction time and applies it after each `save_snapshot` call
...
- [ ] Scenario: snapshot-cleanup — Given 5 snapshots saved with `snapshot_retention_count=3` (from `ClusterConfig`), When cleanup runs after saving the 5th snapshot, Then only the 3 most recent snapshots remain on disk
...
- [ ] Implement commit advancement in leader: after each `FetchRequest` arrives, update `peer.last_fetch_offset` to the `fetch_offset` value from the request; because `fetch_offset` is the **next** index the follower wants (per `architecture.md` §3.2), the highest index the follower has **already replicated** is `last_fetch_offset - 1`; find the highest index I where a majority of voters satisfy `last_fetch_offset - 1 >= I` (i.e., they have persisted entry I) **and** `log[I].term == current_term`, then advance high watermark to I — the leader counts itself as having replicated its own entries
...
- [ ] Implement leader-side snapshot redirect as I/O-free logic in `RaftNode`: when a follower's `last_fetch_offset` is before the log start (entries were compacted), respond to the follower's Fetch with `Action::RedirectToSnapshot { follower_id, snapshot_metadata }` so the driver layer knows to initiate a `FetchSnapshot` stream — the actual gRPC streaming is handled by the driver in Stage 4.2 using the `Transport::send_fetch_snapshot` method from Stage 4.1; this keeps `RaftNode` I/O-free while the snapshot transfer flows through the transport layer
...
- [ ] Scenario: install-snapshot-on-slow-follower — Given a leader that has compacted entries 1-50, When a follower with last_fetch_offset=10 sends a Fetch, Then the leader's `RaftNode` returns `Action::RedirectToSnapshot` and the driver initiates a FetchSnapshot stream via the transport layer; the follower restores from the snapshot
...
### Implementation Steps
- [ ] Implement CLI argument parsing in `xraft-server/src/main.rs` using `clap` (derive API — declared as a workspace dependency in Stage 1.1): define a `Cli` struct with `#[derive(Parser)]` accepting `--config <path>` (required — path to TOML config file) and `--node-id <id>` (optional — override node ID from config); validate that the config path exists and print a usage message on invalid arguments
- [ ] Implement config loading in `xraft-server/src/main.rs`: read the TOML file at the CLI-provided path, deserialize into `NodeConfig` (defined in Stage 1.2), apply environment variable overrides, extract `ClusterConfig` via `into_cluster_config()`, and validate configuration (e.g. node_id is in voter set or observer set)
- [ ] Implement storage initialization in `xraft-server/src/main.rs`: create `data_dir` if it does not exist, initialize `FileLogStore`, `FileHardStateStore`, and `FileSnapshotStore` from `ClusterConfig.data_dir`; if a snapshot exists, load it and restore the `StateMachine`
- [ ] Implement `RaftNode` creation and transport startup in `xraft-server/src/main.rs`: create `RaftNode` from `ClusterConfig` and recovered persistent state (`HardState`, log entries), initialize the `StateMachine` (default `NoOpStateMachine` or consumer-provided), start the gRPC transport server, and initialize the `ConnectionPool` for peer RPCs
- [ ] Implement the driver-loop entry point in `xraft-server/src/main.rs`: wire the initialized `RaftNode`, transport, storage, and state machine into the `DriverLoop` (from Stage 4.2) and run it as the main async task; this is the final step that makes the server operational
...
### Implementation Steps
- [ ] Implement Check Quorum mechanism in leader: track last successful communication time per voter peer; the leader counts itself as one voter and steps down if a majority of the full voter set (including self) has not responded within the `check_quorum_interval` — a separate configurable interval, typically 2× the election timeout (per `architecture.md` §2.1 and `e2e-scenarios.md` Feature 10), distinct from the election timeout itself — e.g. in a 5-node cluster, the leader needs at least 2 of the 4 other voters to have responded within the check quorum interval (since leader + 2 = 3 = majority of 5)
- [ ] Implement leader lease optimization: if the leader has heard from a majority within the last `check_quorum_interval` (via incoming Fetch RPCs), it may skip the extra commit-index confirmation round-trip when answering internal read queries (e.g. admin status queries or `StateMachine`-based lookups); this is a leader-internal optimization and does not expose an external client read API
- [ ] Wire `enable_check_quorum` and `enable_leader_lease` from `ClusterConfig` (defined in Stage 1.2) into the `RaftNode` constructor, gating Check Quorum and leader lease behavior behind these flags; add `check_quorum_interval_ms` derived field computed as 2× `election_timeout_max_ms` (per `architecture.md` §2.1)
...
### Test Scenarios
- [ ] Scenario: check-quorum-steps-down — Given a 3-node cluster where the leader is partitioned from both followers, When the `check_quorum_interval` (2× election timeout) elapses without majority contact, Then the leader steps down to Follower
...
- [ ] Implement `AddVoter`/`RemoveVoter` command rejection: any attempt to issue an `AddVoter` or `RemoveVoter` command must return an `UNSUPPORTED` error with a message indicating dynamic membership is **out of scope for v1** and deferred to a future story entirely — all sibling documents agree on this scoping: `architecture.md` §2.1/§10 says "out of scope for v1 and deferred to a future story entirely", `tech-spec.md` §2.7 says "out of scope for v1 and deferred to a future story entirely — it is not a stretch goal within XRAFT", and `e2e-scenarios.md` Feature 12 adopts the same wording; the voter set remains static for v1
