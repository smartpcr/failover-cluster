...
- [ ] Add Prometheus metrics endpoint at `/metrics` using `axum` and `prometheus-client`, exposing an MVP metrics subset drawn exclusively from the `architecture.md` §7 canonical table (per `e2e-scenarios.md` Feature 15 phased delivery): `xraft_current_term` (gauge), `xraft_commit_index` (gauge), `xraft_current_leader` (gauge — node ID of current leader, -1 if unknown), `xraft_election_latency_seconds` (histogram — time from candidacy to leader election), `xraft_append_records_total` (counter — total entries appended) — the remaining canonical metrics from `architecture.md` §7 are added in Stage 7.1 (`xraft_replication_lag`, `xraft_commit_latency_seconds`, `xraft_fetch_requests_total`) and Stage 7.3 (`xraft_snapshot_installs_total`, `xraft_log_end_offset`) (*cross-doc note:* `e2e-scenarios.md` Feature 15's inline comment at lines 666–668 and Alignment Notes at lines 765–767 reference `xraft_role` as part of the MVP subset, but `xraft_role` does not appear in the `architecture.md` §7 canonical metrics table; this plan follows the authoritative `architecture.md` §7 table — if a role metric is desired, `e2e-scenarios.md` and `architecture.md` should align on adding it to the canonical table first)
