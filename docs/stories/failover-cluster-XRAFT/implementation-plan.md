...
- [ ] Implement `FileHardStateStore` in `xraft-storage/src/state.rs` that persists `HardState` as JSON to a `quorum-state` file with atomic write (write to temp file then rename) for crash safety, consistent with KRaft's `quorum-state` pattern (per `tech-spec.md` §5.3)
  - **Note (Stage 6.1 update):** the canonical `FileHardStateStore` that wires into the server lifecycle now lives in `xraft-storage/src/hard_state.rs` and implements `xraft_core::storage::HardStateStore`. The original `xraft-storage/src/state.rs` defined its own incompatible trait + `HardState` shape (with a `commit` field), is **not** declared in `xraft-storage/src/lib.rs`, and is preserved unmodified as an archived Stage 2.2 artifact. `xraft-storage/src/lib.rs:10` re-exports the canonical engine traits as `pub use xraft_core::storage::{HardStateStore, LogStore, SnapshotStore};` (the `HardState` *struct* is consumed directly from `xraft_core::storage` by downstream crates, not re-exported through `xraft-storage`), and line 31 re-exports the file/memory implementations as `pub use hard_state::{FileHardStateStore, MemoryHardStateStore};`.
...
- [ ] Implement signal handling: `SIGTERM`/`SIGINT` trigger graceful shutdown, `SIGHUP` reloads configuration (the running [`Server`] re-reads the TOML, re-validates `node_id` membership, and swaps the hot-reloadable fields; non-hot-reloadable fields like `listen_addr`, `voters`, `data_dir`, and `admin_listen_addr` are logged-and-ignored; the `RUST_LOG`-derived `tracing` `EnvFilter` is also rebuilt and hot-swapped via `tracing_subscriber::reload::Layer` so log-level changes take effect without a restart)
