...
| `HardStateStore` (trait) | `persist(state: &HardState) -> Result<()>`, `load() -> Result<Option<HardState>>`. The trait operates on `HardState` only; `VoterSet` persistence is a concern of the concrete `FileHardStateStore` implementation (per `implementation-plan.md` Stage 2.2). |
| `FileHardStateStore` | Implements `HardStateStore` and additionally provides concrete (non-trait) methods `persist_with_voters(state: &HardState, voters: &VoterSet) -> Result<()>` and `load_with_voters() -> Result<Option<(HardState, VoterSet)>>` for bootstrap and recovery paths that need both. Atomic write to `quorum-state` file (write-tmp + rename). Stores `HardState` fields and a `voter_set` field as a single JSON object (per `implementation-plan.md` Stage 2.2). |
...
| `NodeConfig` | TOML configuration loaded at startup. `NodeConfig` is a superset of `ClusterConfig` (per `implementation-plan.md` Stage 1.2), embedding `ClusterConfig` via a `#[serde(flatten)]` field and adding extension fields. **`ClusterConfig` fields** (passed to `RaftNode` via `NodeConfig::into_cluster_config()`): `node_id`, `cluster_id`, `data_dir`, `listen_addr`, `peers`, `bootstrap_voters` (parsed into `VoterSet` at startup), `bootstrap_observers`, `election_timeout_min_ms`, `election_timeout_max_ms`, `tick_interval_ms`, `fetch_interval_ms`, `snapshot_interval`, `max_log_entries_before_compaction`, `enable_check_quorum` (default true), `enable_leader_lease` (default false), `segment_max_bytes` (default 64 MiB), `snapshot_max_chunk_bytes` (default 1 MiB), `snapshot_retention_count` (u32, default 3 — number of most-recent snapshots to retain after cleanup; used by `FileSnapshotStore` per Stage 2.3), and optional TLS fields (`tls_enabled`, `tls_cert_path`, `tls_key_path`). **`NodeConfig` extension fields** (not in `ClusterConfig`): `connect_timeout_ms` (default 500), `request_timeout_ms` (default 2000), `admin_listen_addr` (default `"0.0.0.0:9090"`). See §8 for the full reference. |
...
> **Alignment with `implementation-plan.md` Stage 3.3:** Both this document and
> the implementation plan use the same formulation: the leader finds the highest
> index `I` where a majority of voters satisfy `last_fetch_offset - 1 >= I`
> (i.e., they have persisted entry `I`) **and** `log[I].term == current_term`,
> then advances the high watermark to `I`.
...
├── snapshots/                                # zero-padded term (10 digits) + index (20 digits) for sort order
...
// The HardStateStore trait operates on HardState only (per
// implementation-plan.md Stage 2.2). VoterSet persistence is handled
// by the concrete FileHardStateStore, which provides additional
// non-trait methods persist_with_voters() and load_with_voters()
// for bootstrap and recovery paths that need atomic HardState+VoterSet
// access. FileHardStateStore writes both as a single JSON object
// via write-tmp-then-rename for crash safety.
...
chunks. The first chunk carries `SnapshotMetadata` (the protobuf message
with fields `last_included_index`, `last_included_term`, and `voter_set` —
per `implementation-plan.md` Stage 1.3). Each subsequent
...
| `xraft_role` | Gauge | Numeric encoding of the node's current `NodeRole`: 0=Follower, 1=Candidate, 2=PreCandidate, 3=Leader, 4=Observer (per `implementation-plan.md` Stage 6.1 and `e2e-scenarios.md` Feature 15). |
...
#   segment_max_bytes, snapshot_max_chunk_bytes, snapshot_retention_count,
...
snapshot_retention_count = 3         # retain N most-recent snapshots (per implementation-plan.md Stage 2.3)
...
- **`implementation-plan.md`**: Breaks this architecture into ordered implementation phases with crate-level milestones. References component names from this document. Where `implementation-plan.md` uses the name `InstallSnapshot`, it refers to the same operation as `FetchSnapshot` defined here. **Configuration boundary (Stage 1.2):** `ClusterConfig` contains all consensus-relevant fields including `bootstrap_voters`, `bootstrap_observers`, `enable_check_quorum`, `enable_leader_lease`, `segment_max_bytes`, `snapshot_max_chunk_bytes`, `snapshot_retention_count`, and TLS fields. `NodeConfig` embeds `ClusterConfig` via `#[serde(flatten)]` and adds only three deployment-specific extension fields: `connect_timeout_ms`, `request_timeout_ms`, and `admin_listen_addr`. This boundary is consistent between this document (§2.4 and §8) and `implementation-plan.md` Stage 1.2. **Proto ownership:** `proto/raft.proto`, `build.rs`, and the generated Protobuf types all live in `xraft-core` (per `implementation-plan.md` Stage 1.3); `xraft-core/src/message.rs` re-exports the generated types. `xraft-transport` imports these proto types from `xraft-core` and provides the runtime gRPC server/client implementation (see §2.3). **Trait locations:** `implementation-plan.md` Stage 2.1 and Stage 2.3 confirm that all trait definitions (`LogStore`, `SnapshotStore`) live in `xraft-core`; implementation crates import and implement them (see §4.1). **Trait signatures:** §4.1 method names and shapes are aligned with `implementation-plan.md` — `LogStore` uses `append`, `get`, `get_range`, `last_index`, `last_term`, `truncate_from`, `term_at` (Stage 2.1); `SnapshotStore` uses `save_snapshot`, `load_latest_snapshot`, `list_snapshots`, `delete_snapshot(id)` (Stage 2.3); `Transport` uses `send_vote`, `send_pre_vote`, `send_fetch`, `send_fetch_snapshot`, `start_server` (Stage 4.1). **HardStateStore signature:** The `HardStateStore` trait defines `persist(state: &HardState) -> Result<()>` and `load() -> Result<Option<HardState>>` — operating on `HardState` only. `VoterSet` persistence is handled by the concrete `FileHardStateStore`, which provides non-trait methods `persist_with_voters(state: &HardState, voters: &VoterSet)` and `load_with_voters() -> Result<Option<(HardState, VoterSet)>>` for bootstrap and recovery paths. This matches `implementation-plan.md` Stage 2.2, which defines the trait as HardState-only and places `VoterSet` handling in the concrete implementation. **HardState fields:** `HardState` contains three fields: `current_term: Term`, `voted_for: Option<NodeId>`, and `commit_index: LogIndex` (per `implementation-plan.md` Stage 2.2 and Stage 7.2). All three are persisted atomically to the `quorum-state` file. `VoterSet` is persisted alongside `HardState` in the same file by `FileHardStateStore`, but is not part of the trait contract. `last_applied` is volatile, rebuilt from the log on recovery by replaying committed entries from `commit_index`. **EntryPayload alignment:** This document defines `EntryPayload` with four variants: `Command(Bytes)`, `NoOp`, `ConfigChange(VoterSet)`, and `Snapshot(SnapshotMeta)`. This matches `implementation-plan.md` Stage 1.2. The first three variants map 1:1 to the protobuf `LogEntry.entry_type` wire values (`Command`, `NoOp`, `Config`) per Stage 1.3. The fourth variant (`Snapshot`) is an in-memory compaction marker only — never serialised to the wire or written to segment files. Snapshot data is transferred via `FetchSnapshot` RPCs (see §2.3 and §5.3). **Commit advancement (§3.2 / Stage 3.3):** Both this document and `implementation-plan.md` Stage 3.3 use the same formulation: the leader finds the highest index `I` where a majority of voters satisfy `last_fetch_offset - 1 >= I` and `log[I].term == current_term`, then advances the high watermark to `I`. **Log implementation naming:** the concrete log implementation is named `FileLogStore` (per `implementation-plan.md` Stage 2.1), backed by append-only segment files. **Segment default size:** 64 MiB (per `implementation-plan.md` Stage 2.1). **NodeId type:** `NodeId` is `u64` everywhere (per `implementation-plan.md` Stage 1.2). **Snapshot filename convention:** this document adopts the `snapshot-{term}-{index}.bin` naming from `implementation-plan.md` Stage 2.3, adding fixed-width zero-padding for lexicographic ordering on disk (see §2.2 and §3.3). **Snapshot metadata alignment:** This document defines two levels of snapshot metadata. `SnapshotMeta` (core, §3.1) has two fields: `last_included_index` and `last_included_term` — used by `EntryPayload::Snapshot`, `Action::TakeSnapshot`, and `Action::InstallSnapshot`. `SnapshotFileHeader` (on-disk/wire, §3.1) extends `SnapshotMeta` with `voter_set` and `data_len`, matching `implementation-plan.md` Stage 2.3's on-disk header format `[magic][version][last_included_index][last_included_term][voter_set_len][voter_set][data_len]`. The protobuf `SnapshotMetadata` message (Stage 1.3) carries `last_included_index`, `last_included_term`, and `voter_set` — corresponding to the `SnapshotFileHeader` minus `data_len` (which is implicit in stream framing). The first `FetchSnapshotChunk` carries this `SnapshotMetadata` in its `metadata` field (see §5.3).
