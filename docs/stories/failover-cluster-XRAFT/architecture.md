...
| `EntryPayload` | Enum with three canonical variants: `Command(Bytes)`, `NoOp`, `ConfigChange(VoterSet)`. These map 1:1 to the protobuf `LogEntry.entry_type` wire values (`Command`, `NoOp`, `Config`) per `implementation-plan.md` Stage 1.3. `ConfigChange` is reserved for forward-compatible log format; the `RaftNode` rejects any `ConfigChange` proposal with `UNSUPPORTED` in v1. Dynamic membership (`AddVoter`/`RemoveVoter`) is **out of scope for v1** (per `tech-spec.md` §2.7/§3/§7). Snapshots are **not** a log-entry payload — `SnapshotMeta` is a standalone struct (`{ last_included_index: LogIndex, last_included_term: Term }`) used by `SnapshotStore`, `Action::TakeSnapshot`, `Action::InstallSnapshot`, and `FetchSnapshot` RPCs; snapshot data is transferred via `FetchSnapshot` RPCs, not as log entries (see §2.3 and §5.3). This three-variant model matches `implementation-plan.md` Stage 1.2 exactly. |
...
| `HardStateStore` (trait) | `persist(state: &HardState, voters: &VoterSet) -> Result<()>`, `load() -> Result<Option<(HardState, VoterSet)>>`. Atomically persists both `HardState` and `VoterSet` together so the durable quorum state is always consistent (per `implementation-plan.md` Stage 2.2). |
| `FileHardStateStore` | Implements `HardStateStore`. Atomic write to `quorum-state` file (write-tmp + rename). Stores both `HardState` fields and a `voter_set` field as a single JSON object (per `implementation-plan.md` Stage 2.2). |
...
// HardState and VoterSet are always persisted and loaded together
// to keep durable quorum state atomically consistent (per
// implementation-plan.md Stage 2.2). FileHardStateStore writes both
// as a single JSON object via write-tmp-then-rename for crash safety.
