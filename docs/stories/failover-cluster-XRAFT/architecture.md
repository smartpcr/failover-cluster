...
| `EntryPayload` | Enum: `Command(Bytes)`, `NoOp`, `ConfigChange(VoterSet)`, `Snapshot(SnapshotMeta)`. The first three variants map 1:1 to the protobuf `LogEntry.entry_type` wire values (`Command`, `NoOp`, `Config`) per `implementation-plan.md` Stage 1.3. The fourth variant, `Snapshot(SnapshotMeta)`, is an **in-memory compaction marker only** — it is never serialised to the wire or written to segment files; snapshot data is transferred via `FetchSnapshot` RPCs (see §2.3 and §5.3). `ConfigChange` is reserved for forward-compatible log format; the `RaftNode` rejects any `ConfigChange` proposal with `UNSUPPORTED` in v1. Dynamic membership (`AddVoter`/`RemoveVoter`) is **out of scope for v1** (per `tech-spec.md` §2.7/§3/§7). This four-variant model matches `implementation-plan.md` Stage 1.2 exactly. |
...
| `HardStateStore` (trait) | `persist(state: &HardState) -> Result<()>`, `load() -> Result<Option<HardState>>`. Persists `HardState` atomically (per `implementation-plan.md` Stage 2.2). The trait operates on `HardState` only; `VoterSet` persistence is a concern of the concrete implementation (see `FileHardStateStore` below and §4.1). |
| `FileHardStateStore` | Implements `HardStateStore`. Atomic write to `quorum-state` file (write-tmp + rename). The concrete implementation additionally persists `VoterSet` as a separate top-level field in the same `quorum-state` file (per `implementation-plan.md` Stage 7.2), ensuring durable voter-set consistency without coupling the trait to membership. |
...
// Note: The trait persists HardState only. The concrete FileHardStateStore
// additionally writes VoterSet as a separate top-level field in the same
// `quorum-state` file (per implementation-plan.md Stage 7.2), using a
// write-tmp-then-rename operation for crash safety.
